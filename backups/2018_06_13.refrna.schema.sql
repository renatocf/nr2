...
CREATE FUNCTION download_fasta(id_parent bigint, id_classes bigint[]) RETURNS TABLE(sequence json, organism json, databases json, classes json)
    LANGUAGE sql IMMUTABLE STRICT
    AS $_$ SELECT ROW_TO_JSON(sequences.*), ROW_TO_JSON(organisms.*), ARRAY_TO_JSON(ARRAY_AGG(databases.*)), ARRAY_TO_JSON(ARRAY_AGG(classes.*)) FROM organisms_have_taxa JOIN sequences_have_organisms USING (id_organism) JOIN sequences_have_databases USING(id_sequence) JOIN sequences_have_classes USING (id_sequence) JOIN sequences USING (id_sequence) JOIN organisms USING (id_organism) JOIN databases USING (id_database) JOIN classes USING (id_class) WHERE organisms_have_taxa.id_taxon = $1 AND sequences_have_classes.id_class = ANY($2) GROUP BY sequences.*, organisms.*; $_$;
...
CREATE FUNCTION get_children(id_parent bigint, id_classes bigint[])
       RETURNS SETOF taxa
       LANGUAGE sql
       IMMUTABLE STRICT
AS $_$
    SELECT taxa.*
      FROM taxa
      JOIN taxa_have_taxa ON (taxa.id_taxon = taxa_have_taxa.id_child)
      JOIN taxas_classes ON (taxa_have_taxa.id_child = taxas_classes.id_taxon)
     WHERE taxa_have_taxa.id_parent = $1
       AND taxas_classes.id_classes && $2
  ORDER BY taxa.taxon;
$_$;
...
--
-- Name: search_taxon(text, integer); Type: FUNCTION; Schema: refrna; Owner: refrna
--

CREATE FUNCTION search_taxon(input text, maximum integer) RETURNS SETOF taxa
    LANGUAGE sql
    AS $$ SELECT taxa FROM taxa, to_tsvector(taxa.taxon) AS target, phraseto_tsquery(input) AS query, ts_rank_cd(target, query) AS rank WHERE target @@ query ORDER BY rank DESC LIMIT maximum; $$;


ALTER FUNCTION refrna.search_taxon(input text, maximum integer) OWNER TO refrna;
...
CREATE MATERIALIZED VIEW taxas_classes AS
...
ALTER TABLE taxas_classes OWNER TO refrna;
...
--
-- Name: taxas_classes; Type: ACL; Schema: refrna; Owner: refrna
--

GRANT SELECT ON TABLE taxas_classes TO anon;
