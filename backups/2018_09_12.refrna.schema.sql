...
CREATE FUNCTION refrna.get_children(id_parent bigint, id_classes bigint[]) RETURNS TABLE(id_taxon bigint, taxon text, num_sequences bigint)
    LANGUAGE sql IMMUTABLE STRICT
    AS $_$
    SELECT taxa.*, sum(taxa_classes_sequences.num_sequences)::bigint
      FROM taxa
      JOIN taxa_have_taxa ON (taxa.id_taxon = taxa_have_taxa.id_child)
      JOIN taxa_classes_sequences ON (taxa_have_taxa.id_child = taxa_classes_sequences.id_taxon)
     WHERE taxa_have_taxa.id_parent = $1
       AND taxa_classes_sequences.id_class = ANY($2)
...
CREATE FUNCTION refrna.get_root(id_classes bigint[]) RETURNS TABLE(id_taxon bigint, taxon text, num_sequences bigint)
    ...
    SELECT taxa.*, sum(taxa_classes_sequences.num_sequences)::bigint
      FROM taxa
      JOIN taxa_classes_sequences USING (id_taxon)
     WHERE taxa.id_taxon = 0
       AND taxa_classes_sequences.id_class = ANY($1)
  GROUP BY taxa.id_taxon;
$_$;
...
--
-- Name: taxa_classes_sequences; Type: MATERIALIZED VIEW; Schema: refrna; Owner: refrna
--

CREATE MATERIALIZED VIEW refrna.taxa_classes_sequences AS
 SELECT organisms_have_taxa.id_taxon,
    sequences_have_classes.id_class,
    count(DISTINCT sequences_have_classes.id_sequence) AS num_sequences
   FROM ((refrna.sequences_have_classes
     JOIN refrna.sequences_have_organisms USING (id_sequence))
     JOIN refrna.organisms_have_taxa USING (id_organism))
  GROUP BY organisms_have_taxa.id_taxon, sequences_have_classes.id_class
  WITH NO DATA;


ALTER TABLE refrna.taxa_classes_sequences OWNER TO refrna;
...
--
-- Name: TABLE taxa_classes_sequences; Type: ACL; Schema: refrna; Owner: refrna
--

GRANT SELECT ON TABLE refrna.taxa_classes_sequences TO anon;
