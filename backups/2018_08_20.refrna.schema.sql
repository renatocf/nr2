...
CREATE FUNCTION refrna.get_children(id_parent bigint, id_classes bigint[]) RETURNS TABLE(id_taxon bigint, taxon text, num_sequences bigint)
    LANGUAGE sql IMMUTABLE STRICT
    AS $_$
    SELECT taxa.*, count(sequences_have_organisms.id_organism)
      FROM taxa
      ...
      JOIN organisms_have_taxa ON (taxa_have_taxa.id_child = organisms_have_taxa.id_taxon)
      JOIN sequences_have_organisms USING (id_organism)
     WHERE taxa_have_taxa.id_parent = $1
       AND taxas_classes.id_classes && $2
  GROUP BY taxa.id_taxon, organisms_have_taxa.id_taxon
  ORDER BY taxa.taxon;
$_$;
...
CREATE FUNCTION refrna.get_root(id_classes bigint[]) RETURNS TABLE(id_taxon bigint, taxon text, num_sequences bigint)
    LANGUAGE sql
    AS $_$ SELECT taxa.id_taxon, taxa.taxon, (SELECT count(*) FROM sequences_have_classes WHERE id_class = ANY($1)) AS num_sequences FROM taxa WHERE taxa.id_taxon = 0; $_$;
...
CREATE FUNCTION refrna.search_taxon(input text, id_classes bigint[], maximum integer) RETURNS TABLE(id_taxon bigint, taxon text, num_sequences bigint)
    ...
    SELECT taxa.*, count(sequences_have_organisms.id_organism)
      ...
      JOIN sequences_have_organisms USING (id_organism),
           ...
           phraseto_tsquery(input) AS query,
           ts_rank_cd(target, query) AS rank
     WHERE target @@ query GROUP BY taxa.id_taxon, rank
  ORDER BY rank DESC
     LIMIT maximum;
$$;
...
CREATE MATERIALIZED VIEW refrna.root_taxa AS
 SELECT taxa.id_taxon,
    taxa.taxon
   FROM (refrna.taxa
     LEFT JOIN refrna.taxa_have_taxa ON ((taxa.id_taxon = taxa_have_taxa.id_child)))
  WHERE (taxa_have_taxa.id_parent IS NULL)
  ORDER BY taxa.taxon
  WITH NO DATA;


ALTER TABLE refrna.root_taxa OWNER TO refrna;

--
-- Name: root_taxon; Type: MATERIALIZED VIEW; Schema: refrna; Owner: refrna
--

CREATE MATERIALIZED VIEW refrna.root_taxon AS
 SELECT taxa.id_taxon,
    taxa.taxon,
    ( SELECT count(*) AS count
           FROM refrna.sequences) AS num_sequences
   FROM refrna.taxa
  WHERE (taxa.id_taxon = 0)
  WITH NO DATA;


ALTER TABLE refrna.root_taxon OWNER TO refrna;
...
--
-- Name: TABLE root_taxa; Type: ACL; Schema: refrna; Owner: refrna
--

GRANT ALL ON TABLE refrna.root_taxa TO anon;


--
-- Name: TABLE root_taxon; Type: ACL; Schema: refrna; Owner: refrna
--

GRANT SELECT ON TABLE refrna.root_taxon TO anon;
